...
| Tab | Chức năng |
|---|---|
| **Nạp từ vựng** | Dán text, parse, tự tra IPA, lưu DB — mỗi lần bấm "Nạp vào hệ thống" là 1 "lần nạp" (Lần 1, Lần 2, Lần 3...) riêng biệt. Có thể đặt tên tuỳ chọn cho lần nạp (vd "Từ vựng Unit 3") trước khi bấm nạp |
...
| **Xem theo lần nạp** | Xem toàn bộ từ của 1 lần nạp cụ thể, không giới hạn. Có nút "Đổi tên" (đặt/sửa/xoá tên lần nạp bất kỳ lúc nào), "Học lại từ đầu (lần này)" để reset tiến độ 3 chế độ về 0, "Xóa cả lần nạp này" nếu nạp nhầm, chọn nhiều từ (checkbox) để xóa hàng loạt, và xóa từng từ riêng lẻ (🗑) — có ở cả tab này lẫn "Danh sách từ vựng" |
| **Kỹ năng** | 6 tab con Reading / Listening / Writing / Writing Ideas / Speaking / Bonus, hiển thị nội dung từ file tương ứng trong thư mục `docs/` (bạn tự chỉnh sửa). Render markdown nhẹ: `#`/`##`/`###` heading, dòng CHỮ HOA, `**bold**` (cả nguyên dòng thành tiêu đề phụ), `~~gạch ngang~~`, `> trích dẫn` (blockquote), bullet (`-`, `•`, `✔️`, `👉`, `📌`, `→`, `->`), dòng `(VN: ...)` nối vào bullet phía trên như ghi chú dịch, `⸻`/`---` thành đường kẻ ngang |
...
```
POST   /api/vocab/import                       Body { text, batchName? } — parse + tra IPA + lưu DB, trả về importBatch (số lần nạp)
PUT    /api/vocab/by-batch/:batch/name          Body { name } — đổi tên lần nạp (name rỗng để bỏ tên)
...
GET    /api/docs/:skill                          Nội dung file docs/<skill>.md — skill: reading/listening/writing/writing-ideas/speaking/bonus
